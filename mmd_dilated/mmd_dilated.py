...
from .sequence_form_utils import construct_vars, sequence_to_policy, policy_to_sequence
from .sequence_form_utils import uniform_random_seq, _get_action_from_key
from .sequence_form_utils import is_root, _EMPTY_INFOSET_ACTION_KEYS, _EMPTY_INFOSET_KEYS
...
def kl_divergence(probs1, probs2):
    # compute kl divergence B(probs1; probs2)
...
class MMDDilatedEnt(object):
    ...
    def __init__(self, game, alpha, stepsize=None):
        ...
        if stepsize is not None:
            ...
            self.stepsize = self.alpha/(max(abs(self.game.max_utility()), abs(self.game.min_utility()))**2)
    ...
    def get_gap(self):
        """
        Computes saddle point game of the regularized game.
